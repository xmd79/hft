...
LEVERAGE = 125                     # Leverage to set (changed from 75 to 125)
...
async def run_backtest(async_client):
    ...
    global current_balance, backtest_results, candle_data, price_history, lrc_prices
...
def print_main_status(latest_row, forecast_price, forecast_delta, signals):
    try:
        ...
        if latest_row is None:
            print(f"[{ts}] No candle yet | ForecastΔ: {forecast_delta:+.4f} | ForecastPrice: {forecast_price:.2f} | Signals: none | Balance: ${current_balance:.4f}")
            return
        ...
        print(f"[{ts}] Price: {price:.2f} | Vol: {vol:.6f} | Mom: {momentum:.4f} | EMAf: {ema_f:.2f} | EMAs: {ema_s:.2f} | StochK: {stoch_k:.1f} | Regime: {market_regime} | Volatility: {volatility_state} | {arctanh_text} | {fft_text} | {lrc_text} | ForecastΔ: {forecast_delta:+.4f} | Forecast: {forecast_price:.2f} | Signals: {sig_text} | Open: {open_count} | Balance: ${current_balance:.4f}")
    except Exception as e:
