...
async def execute_trade(async_client, action, entry_price, quantity, stop_loss, take_profit, can_trade, df: pd.DataFrame):
    """
    Execute a real trade (async) or simulate it. Adds trade to open_trades.
    Implements trailing stop for trending markets.
    """
    global virtual_balance, open_trades
    ...
    print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] ENTRY -> {action.upper():4} | Qty {quantity:.6f} | Price {entry_price:.2f} | SL {stop_loss:.2f} | TP {take_profit:.2f} | Simulated: {trade_info['is_simulated']}")
    ...
    for trade in open_trades:
        ...
        if action == 'buy':
            if current_price <= stop_loss:
                ...
                closed.append(trade)
                print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] EXIT  -> BUY closed at ${current_price:.2f} | PnL ${pnl:.4f} | Simulated: {is_simulated}")
                if not is_simulated:
                    virtual_balance += pnl
            ...
            if current_price >= stop_loss:
                ...
                closed.append(trade)
                print(f"[{datetime.now(timezone.utc).strftime('%H:%M:%S')}] EXIT  -> SELL closed at ${current_price:.2f} | PnL ${pnl:.4f} | Simulated: {is_simulated}")
                if not is_simulated:
                    virtual_balance += pnl
                    # Record backtest result
                    if BACKTEST_MODE:
...
async def run_backtest(async_client):
    ...
    for kline in klines:
        ...
        for action, entry_price, intensity_pct, pname in signals:
            ...
            await execute_trade(async_client, action, entry_price, quantity, stop_loss, take_profit, False, df)
                    ...
                    if intensity_pct >= 80:
                        ...
                        if atr > 0:
                            ...
                            if action == 'buy':
                                ...
                                stop_loss = entry_price + (atr * STOP_LOSS_ATR_MULTIPLIER)
                                take_profit = entry_price - price_move
                    await execute_trade(async_client, action, entry_price, quantity, stop_loss, take_profit, can_trade, df)
