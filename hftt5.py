...
LEVERAGE = 125                     # Leverage to set (increased to 125x)
...
MIN_PROFIT_TARGET = 0.0075         # Minimum profit target (0.75%)
EXTREME_LOOKBACK = 200             # Number of candles to look back for extremes
EXTREME_CONFIRMATION_THRESHOLD = 0.001  # 0.1% confirmation threshold
...
bot_start_time = time.time()       # Track when the bot started running
...
# ---------------------------
# Extreme Detection Functions
# ---------------------------
def find_recent_extremes(df, lookback=EXTREME_LOOKBACK):
    """Find the most recent significant low (argmin) and high (argmax) in the lookback period."""
    if len(df) < lookback:
        lookback = len(df)
    
    recent_df = df.iloc[-lookback:]
    
    # Find the index of the minimum low and maximum high
    ...
    # Calculate how many candles ago these extremes occurred
    low_candles_ago = len(df) - low_idx - 1
    high_candles_ago = len(df) - high_idx - 1
    
    return {
        'low': {'price': low_price, 'candles_ago': low_candles_ago, 'index': low_idx},
        'high': {'price': high_price, 'candles_ago': high_candles_ago, 'index': high_idx}
    }

def is_valid_reversal_signal(df, action, extremes):
    """Check if the current price action forms a valid reversal signal based on extremes."""
    if df.empty:
        return False
    ...
    if action == 'buy':
        # For long signals, check if we have a recent significant low
        if extremes['low']['candles_ago'] < EXTREME_LOOKBACK:
            low_price = extremes['low']['price']
            # Check if current price is above the low by at least the confirmation threshold
            if current_price > low_price * (1 + EXTREME_CONFIRMATION_THRESHOLD):
                return True
    else:  # action == 'sell'
        # For short signals, check if we have a recent significant high
        if extremes['high']['candles_ago'] < EXTREME_LOOKBACK:
            high_price = extremes['high']['price']
            # Check if current price is below the high by at least the confirmation threshold
            if current_price < high_price * (1 - EXTREME_CONFIRMATION_THRESHOLD):
                return True
...
def detect_patterns_with_intensity(df):
    ...
    extremes = find_recent_extremes(df)
    ...
    if (time_valid and spike_ok and base_ok and confirm_candle_ok and
            ...
            is_valid_reversal_signal(df, 'sell', extremes)):
    ...
    if (time_valid and spike_ok and base_ok and confirm_candle_ok and
            ...
            is_valid_reversal_signal(df, 'buy', extremes)):
        intensity = pattern_intensity(df, i)
    ...
    if last_mom > MOMENTUM_THRESHOLD * 3:
        ...
        if is_valid_reversal_signal(df, direction, extremes):
...
async def calculate_quantity_and_levels(entry_price: float, action: str, async_client, df: pd.DataFrame) -> tuple:
    ...
    # Calculate take profit distance with minimum profit target
    min_profit_distance = entry_price * MIN_PROFIT_TARGET
    take_profit_distance = max(atr * TAKE_PROFIT_ATR_MULTIPLIER, min_profit_distance)
...
async def execute_trade(async_client, action, entry_price, quantity, stop_loss, take_profit, can_trade, df: pd.DataFrame, pattern_name='', intensity=0):
    ...
    # Update virtual balance for both simulated and real trades
    virtual_balance -= margin_required
    
    # Adjust stop loss based on extreme prices for reversal patterns
    if pattern_name in ['DBR', 'RBR', 'SpikeBuy']:  # Long reversal patterns
        extremes = find_recent_extremes(df)
        recent_low = extremes['low']['price']
        # Use the recent low as stop loss if it's closer than the ATR-based stop
        if recent_low > stop_loss:  # For long, stop_loss should be below entry
            stop_loss = recent_low * 0.999  # Slightly below the low to avoid noise
    elif pattern_name in ['DBD', 'RBD', 'SpikeSell']:  # Short reversal patterns
        extremes = find_recent_extremes(df)
        recent_high = extremes['high']['price']
        # Use the recent high as stop loss if it's closer than the ATR-based stop
        if recent_high < stop_loss:  # For short, stop_loss should be above entry
            stop_loss = recent_high * 1.001  # Slightly above the high to avoid noise
    ...
    if can_trade and async_client and AUTH:
        try:
            ...
            logger.info(f"Real trade placed: {action.upper()} {quantity} {SYMBOL} at {entry_price:.2f} | SL {stop_loss:.2f} TP {take_profit:.2f} | margin used: ${margin_required:.4f} | Balance: ${virtual_balance:.4f}")
        except Exception as e:
        ...
        logger.info(f"Simulating trade due to {reason}: {action.upper()} {quantity} {SYMBOL} at ${entry_price:.2f}, SL: ${stop_loss:.2f}, TP: ${take_profit:.2f} | Balance: ${virtual_balance:.4f}")

    open_trades.append(trade_info)
    print(f"[{datetime.now().strftime('%H:%M:%S')}] ENTRY -> {action.upper():4} | Qty {quantity:.6f} | Price {entry_price:.2f} | SL {stop_loss:.2f} | TP {take_profit:.2f} | Simulated: {trade_info['is_simulated']} | Pattern: {pattern_name} | Balance: ${virtual_balance:.4f}")
    ...
    for trade in open_trades:
        ...
        if action == 'buy':
            if current_price <= stop_loss:
                ...
                closed.append(trade)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] EXIT  -> BUY closed at ${exit_price:.2f} | PnL ${pnl:.4f} | Balance: ${virtual_balance + pnl + margin_used:.4f}")
                # Update virtual balance for both simulated and real trades
                virtual_balance += pnl + margin_used
            ...
            if current_price >= stop_loss:
                ...
                closed.append(trade)
                print(f"[{datetime.now().strftime('%H:%M:%S')}] EXIT  -> SELL closed at ${exit_price:.2f} | PnL ${pnl:.4f} | Balance: ${virtual_balance + pnl + margin_used:.4f}")
                # Update virtual balance for both simulated and real trades
                virtual_balance += pnl + margin_used
            elif current_price <= take_profit:
                exit_price = take_profit
...
def print_periodic_report():
    ...
    try:
        ...
        # Calculate running time using bot_start_time
        ...
        print(f"Losing Trades: {len(all_losing_trades)}")
...
def print_main_status(latest_row, forecast_price, forecast_delta, signals):
    try:
        ...
        print(f"[{ts}] Price: {price:.2f} | Vol: {vol:.6f} | Mom: {momentum:.4f} | EMAf: {ema_f:.2f} | EMAs: {ema_s:.2f} | StochK: {stoch_k:.1f} | Regime: {market_regime} | Volatility: {volatility_state} | ForecastΔ: {forecast_delta:+.4f} | Forecast: {forecast_price:.2f} | Signals: {sig_text} | Open: {open_count} | Balance: ${virtual_balance:.4f}")
    except Exception as e:
...
async def main():
    ...
    bot_start_time = time.time()  # Set the start time when the bot begins
