...
def detect_patterns_with_intensity(df, current_price):
    ...
    if (time_valid and spike_ok and base_ok and confirm_candle_ok and
            ...
            (arctanh_signal == "short" or extreme_signal == "short" or fft_confirms_short or lrc_signal == "short")):
    ...
    if (time_valid and spike_ok and base_ok and confirm_candle_ok and
            ...
            (arctanh_signal == "long" or extreme_signal == "long" or fft_confirms_long or lrc_signal == "long")):
        intensity = pattern_intensity(df, i)
        # Adjust intensity based on Hurst exponent
        if USE_HURST and current_hurst is not None:
            if current_hurst > 0.5:  # Trending market
    ...
    if last_mom > MOMENTUM_THRESHOLD * 3:
        ...
        # Only add if confirmed by arctanh, extreme, FFT, or LRC
        if (direction == 'buy' and (arctanh_signal == "long" or extreme_signal == "long" or fft_confirms_long or lrc_signal == "long")) or \
           (direction == 'sell' and (arctanh_signal == "short" or extreme_signal == "short" or fft_confirms_short or lrc_signal == "short")):
...
def print_main_status(latest_row, forecast_price, forecast_delta, signals):
    try:
        ...
        if latest_row is None:
            ...
            print(f"[{ts}] No candle yet | ForecastΔ: {forecast_delta:+.4f} | ForecastPrice: {forecast_price:.2f} | Signals: none | Balance: ${current_balance:.4f} | {hurst_text}")
            return
